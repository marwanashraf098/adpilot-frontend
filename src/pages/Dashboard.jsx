...
function AdSetRow({ adSet }) {
    ...
    if (!expanded && ads.length === 0) {
      ...
      try {
        const res = await axios.get(
          `${BACKEND}/api/campaigns/adsets/${adSet.id}/ads`,
        );
        setAds(res.data);
      } catch (err) {
  ...
  return (
    <>
      <tr
        className="border-b border-gray-800/30 bg-gray-900/50 hover:bg-gray-800/20 transition cursor-pointer"
        onClick={toggleAds}
      >
        ...
        <td className="px-4 py-3">
          ...
          <p className="text-xs text-gray-600 mt-0.5">
            Ad Set · {adSet.optimizationGoal || "N/A"} · Age {adSet.minAge}-
            {adSet.maxAge}
          </p>
        </td>
        <td className="px-4 py-3 text-right text-sm text-gray-400">
          EGP {(adSet.spend || 0).toFixed(2)}
        </td>
        <td className="px-4 py-3 text-right text-sm text-gray-400">
      ...
      {expanded &&
        (loadingAds ? (
          <tr className="bg-gray-950/50">
            <td colSpan={7} className="pl-20 py-2 text-xs text-gray-600">
              Loading ads...
            </td>
          </tr>
        ) : (
          ads.map((ad) => (
            <tr
              key={ad.id}
              className="border-b border-gray-800/20 bg-gray-950/50 hover:bg-gray-900/30 transition"
            >
...
function CampaignRow({ campaign }) {
  ...
  const toggleAdSets = async () => {
    if (!expanded && adSets.length === 0) {
      ...
      try {
        const res = await axios.get(
          `${BACKEND}/api/campaigns/${campaign.id}/adsets`,
        );
        setAdSets(res.data);
      } catch (err) {
    ...
    <>
      ...
      >
        ...
        <td className="px-6 py-4 text-right text-sm">
          EGP {(campaign.spend || 0).toFixed(2)}
        </td>
        <td className="px-6 py-4 text-right text-sm">
      ...
      {expanded &&
        (loadingAdSets ? (
          <tr className="bg-gray-900/50">
            <td colSpan={7} className="pl-12 py-2 text-xs text-gray-600">
              Loading ad sets...
            </td>
          </tr>
        ) : (
...
function Dashboard() {
  ...
  const [actionModal, setActionModal] = useState(null); // {rec, action}
  ...
  useEffect(() => {
    ...
    if (
      ...
      localStorage.getItem("oauthReturnTo") === "onboarding"
    ) {
  ...
  const handleConnectMeta = async () => {
    try {
      const res = await axios.get(
        `${BACKEND}/api/meta/oauth-url?userId=${userId}`,
      );
  ...
  const fetchRecommendations = async (campaignData) => {
    ...
    try {
        ...
        campaignData.map(async (c) => {
          try {
            const adSetsRes = await axios.get(
              `${BACKEND}/api/campaigns/${c.id}/adsets`,
            );
            return {
              name: c.name,
              status: c.status,
              ...
              ad_sets: adSetsRes.data.map((as) => ({
                ...
                platform_ad_set_id: as.platformAdSetId || "",
              })),
            };
          } catch (err) {
            return {
      ...
      const payload = {
        industry:
          business?.industry || localStorage.getItem("industry") || "business",
  ...
  const fetchSuggestions = async (campaignData) => {
    ...
    try {
      const res = await axios.post(`${AI}/generate-campaign-suggestions`, {
        ...
        existing_campaigns: campaignData.map((c) => ({
          name: c.name,
        ...
        avg_ctr:
          campaignData.length > 0
            ? campaignData.reduce((sum, c) => sum + (c.ctr || 0), 0) /
              campaignData.length
            : 0,
      });
  ...
  const executeAction = async (rec) => {
    if (!rec.action || rec.action.type === "NONE") return;
    setActionModal({ rec });
  };

  const confirmAction = async () => {
    const { rec } = actionModal;
    setExecutingAction(true);
    try {
      const res = await axios.post(
        `${BACKEND}/api/campaigns/execute-action?userId=${userId}`,
        {
          type: rec.action.type,
          entityId: rec.action.entityId,
          level: rec.action.level,
          newValue: rec.action.newValue,
        },
      );
      ...
      <div className="max-w-6xl mx-auto px-6 py-8">
        ...
        <div className="flex items-center justify-between mb-8">
          ...
          <div className="flex items-center gap-3">
            ...
            <button
              ...
              className="bg-green-600 hover:bg-green-700 text-white text-sm font-semibold rounded-lg px-4 py-2.5 transition"
            >
              + Create Campaign
            </button>
            <button
              onClick={() => navigate("/competitor-spy")}
              className="bg-gray-800 hover:bg-gray-700 text-white text-sm font-semibold rounded-lg px-4 py-2.5 transition"
            >
              🕵️ Competitor Spy
            </button>
          ...
          {loadingRecs && (
            <div className="bg-gray-900 border border-gray-800 rounded-2xl p-6 text-center">
              <p className="text-gray-500 text-sm">
                AI is analyzing your campaigns, ad sets, and ads...
              </p>
            </div>
          )}
        ...
        <div className="mt-8">
          <div className="flex items-center justify-between mb-4">
            ...
            <button
              onClick={() => fetchSuggestions(campaigns)}
          ...
          {loadingSuggestions && (
            <div className="bg-gray-900 border border-gray-800 rounded-2xl p-6 text-center">
              <p className="text-gray-500 text-sm">
                AI is finding the best campaign opportunities for you...
              </p>
            </div>
          )}
        ...
        <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50 px-4">
          <div className="bg-gray-900 border border-gray-700 rounded-2xl p-6 max-w-md w-full">
            ...
            <p className="text-gray-400 text-sm mb-4">
              {actionModal.rec.title}
            </p>
            <div className="bg-gray-800 rounded-lg px-4 py-3 mb-4">
              <p className="text-xs text-gray-400 mb-1">Action</p>
              <p className="text-sm font-medium text-white">
                {actionModal.rec.action.type.replace(/_/g, " ")}
              </p>
              {actionModal.rec.action.newValue && (
                <p className="text-xs text-gray-400 mt-1">
                  New budget: EGP {actionModal.rec.action.newValue}/day
                </p>
              )}
              <p className="text-xs text-gray-500 mt-1">
                Confidence: {actionModal.rec.confidence}%
              </p>
            </div>
            <p className="text-xs text-yellow-400 mb-4">
              ⚠ This will make a real change to your Meta campaign.
            </p>
